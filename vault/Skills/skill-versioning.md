...
score: <float|null>                  # populated by artifact-staged-eval; null = not yet evaluated
...
- **`score`** — populated by `Skills/artifact-staged-eval.md` after evaluation. `null` means "not yet evaluated" — the parent-selection skill treats `null` distinctly from a low score.
...
- **Automatic rollback** — `Skills/artifact-staged-eval.md` re-evaluates the live version on a subsequent run and the score drops below the previous live's score by more than the eval's regression tolerance.
...
- `artifact-staged-eval` reads any version's body, populates `score`, and may trigger rollback via §6.
